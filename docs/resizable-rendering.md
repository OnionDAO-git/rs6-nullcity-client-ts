...
4. Add a `resizable` layout mode that computes:
   ...
   - legacy-centered fallback bounds for fixed fullscreen interfaces.
...
5. Enable a hidden `?layout=resizable` mode for development.
...
- The default output remains pixel-identical to the old fixed frame.

## Foundation Status

- Legacy mode remains the default.
- `?layout=resizable` enables a hidden development path.
...
- `public/index.html` switches the canvas CSS from legacy aspect-fit scaling to full-viewport sizing only when the hidden resizable query flag is present and the active frame is resizable.
...
- `World` uses a larger tile visibility/render span for high-resolution viewports to reduce terrain culling gaps at the expanded viewport edges, with matching far-clip increases in the world/model projection path.
- The loaded terrain area can be expanded with matching client/server settings:
  - Client: `?buildAreaScale=1..4` or `?mapBuildAreaScale=1..4`; the value persists in local storage.
  - Server: `loadedZoneScale` in `server-config.json`.
  - The client and server values must match for scaled terrain streaming. The client can fall back to a smaller area when a legacy rebuild packet is received, but a server configured larger than the client is not supported.
